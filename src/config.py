...
def get_retention_days(env: Any) -> int:
    ...
    return int(get_config_value(env, "RETENTION_DAYS", DEFAULT_RETENTION_DAYS))


def get_max_entries_per_feed(env: Any) -> int:
    """Get maximum entries to keep per feed."""
    return int(get_config_value(env, "MAX_ENTRIES_PER_FEED", DEFAULT_MAX_ENTRIES_PER_FEED))


def get_embedding_max_chars(env: Any) -> int:
    """Get maximum characters to embed per entry."""
    return int(get_config_value(env, "EMBEDDING_MAX_CHARS", DEFAULT_EMBEDDING_MAX_CHARS))
...
def get_search_top_k(env: Any) -> int:
    ...
    return int(get_config_value(env, "SEARCH_TOP_K", DEFAULT_SEARCH_TOP_K))
...
def get_feed_auto_deactivate_threshold(env: Any) -> int:
    ...
    return int(
        get_config_value(
            env, "FEED_AUTO_DEACTIVATE_THRESHOLD", DEFAULT_FEED_AUTO_DEACTIVATE_THRESHOLD
        )
    )
...
def get_feed_failure_threshold(env: Any) -> int:
    ...
    return int(get_config_value(env, "FEED_FAILURE_THRESHOLD", DEFAULT_FEED_FAILURE_THRESHOLD))


def get_feed_timeout(env: Any) -> int:
    """Get per-feed processing timeout in seconds."""
    return int(get_config_value(env, "FEED_TIMEOUT", FEED_TIMEOUT_SECONDS))


def get_http_timeout(env: Any) -> int:
    """Get HTTP request timeout in seconds."""
    return int(get_config_value(env, "HTTP_TIMEOUT", HTTP_TIMEOUT_SECONDS))
