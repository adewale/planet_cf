...
**Testing**: Wrappers have two-tier tests -- CPython mocks (`test_safe_boundary`) and FFI fakes (`test_boundary_ffi.py`) that install a fake CFBoundary Pyodide runtime with fake JS types. Run `uv run pytest tests/unit/test_safe_boundary tests/unit/test_boundary_ffi.py --co -q` for current counts. See [Lesson 20](LESSONS_LEARNED.md#20-two-tier-ffi-testing-cpython-tests--pyodide-fakes).
...
│  │                                                                              │  │
│  │  Imports from all other modules in src/ (see arrows below)                    │  │
│  └──────┬────────────────────────────────────────────────────────────────┬──────┘  │
│         │                                                                │         │
│    ┌────┴────────────┬──────────────┬──────────────┬──────────────┐      │         │
│    ▼                 ▼              ▼              ▼              ▼      │         │
