...
class TestStripDuplicateTitle:
    """Tests for _strip_duplicate_title function.
    ...
    duplication. The function should strip the leading heading if it matches.
    """

    def test_strips_h1_matching_title(self):
        """Strips <h1> at start of content when it matches title."""
        from src.main import _strip_duplicate_title

        title = "Open Graph Images in Astro: Build-Time vs Runtime"
        content = "<h1> Open Graph Images in Astro: Build-Time vs Runtime </h1><p>Content here</p>"
        result = _strip_duplicate_title(content, title)
        assert "<h1>" not in result
        assert "<p>Content here</p>" in result

    def test_strips_h2_matching_title(self):
        """Strips <h2> at start of content when it matches title."""
        from src.main import _strip_duplicate_title

        title = "What even are Cloudflare Durable Objects?"
        content = "<h2>What even are Cloudflare Durable Objects?</h2><p>Let me explain.</p>"
        result = _strip_duplicate_title(content, title)
        assert "<h2>" not in result
        assert "<p>Let me explain.</p>" in result

    def test_handles_whitespace_in_heading(self):
        """Handles extra whitespace inside the heading tag."""
        from src.main import _strip_duplicate_title

        title = "My Post Title"
        content = "<h1>  My Post Title  </h1><p>Body</p>"
        result = _strip_duplicate_title(content, title)
        assert "<h1>" not in result
        assert "<p>Body</p>" in result

    def test_handles_leading_whitespace_before_heading(self):
        """Handles whitespace/newlines before the heading."""
        from src.main import _strip_duplicate_title

        title = "My Post Title"
        content = "   \n  <h1>My Post Title</h1><p>Body</p>"
        result = _strip_duplicate_title(content, title)
        assert "<h1>" not in result
        assert "<p>Body</p>" in result

    def test_case_insensitive_matching(self):
        """Matches title regardless of case differences."""
        from src.main import _strip_duplicate_title

        title = "The Context Is The Work"
        content = "<h1>the context is the work</h1><p>Body</p>"
        result = _strip_duplicate_title(content, title)
        assert "<h1>" not in result
        assert "<p>Body</p>" in result

    def test_preserves_non_matching_h1(self):
        """Does not strip <h1> when it doesn't match the title."""
        from src.main import _strip_duplicate_title

        title = "My Post Title"
        content = "<h1>Different Heading</h1><p>Body</p>"
        result = _strip_duplicate_title(content, title)
        assert "<h1>Different Heading</h1>" in result
        assert "<p>Body</p>" in result

    def test_preserves_h1_in_middle_of_content(self):
        """Does not strip <h1> that appears later in content."""
        from src.main import _strip_duplicate_title

        title = "My Post Title"
        content = "<p>Intro</p><h1>My Post Title</h1><p>Body</p>"
        result = _strip_duplicate_title(content, title)
        assert "<h1>My Post Title</h1>" in result

    def test_strips_heading_with_link(self):
        """Strips heading that contains a link wrapping the title."""
        from src.main import _strip_duplicate_title

        title = "My Post Title"
        content = '<h1><a href="#">My Post Title</a></h1><p>Body</p>'
        result = _strip_duplicate_title(content, title)
        assert "<h1>" not in result
        assert "<p>Body</p>" in result
    ...
    def test_handles_empty_content(self):
        ...
        result = _strip_duplicate_title("", "Title")
        assert result == ""
    ...
    def test_handles_empty_title(self):
        ...
        content = "<h1>Some Heading</h1><p>Body</p>"
        result = _strip_duplicate_title(content, "")
        assert result == content

    def test_handles_none_title(self):
        """Handles None title gracefully."""
        from src.main import _strip_duplicate_title

        content = "<h1>Some Heading</h1><p>Body</p>"
        result = _strip_duplicate_title(content, None)
        assert result == content

    def test_strips_metadata_before_title(self):
        """Strips date/metadata that appears before the title heading."""
        from src.main import _strip_duplicate_title

        title = "the context is the work"
        content = " <h1>the context is the work</h1> 7 January 2026  <p>Body</p>"
        result = _strip_duplicate_title(content, title)
        assert "<h1>" not in result
        # The date after the heading should remain
        assert "7 January 2026" in result or "<p>Body</p>" in result
